...
const detailsLinkContainer = document.getElementById(
...
function resetCourtUI() {
  ...
  courtSelect.innerHTML =
    '<option value="" selected disabled hidden>Выбрать площадку</option>';
...
courtSelect.addEventListener("change", function () {
  ...
  detailsLink.style.color = "#6c5ce7";
  ...
  fetch("/court/timeslots/" + courtId)
    ...
    .then((timeSlots) => {
      ...
      if (days.length === 0) {
        timePills.innerHTML =
          '<div style="padding:10px; font-weight:700;">Нет доступного времени</div>';
      ...
      days.forEach((day, idx) => {
        ...
        btn.type = "button";
        btn.className = "bkng_tab" + (idx === 0 ? " active" : "");
        btn.textContent = day;
        btn.onclick = () => {
          document
            .querySelectorAll(".bkng_tab")
